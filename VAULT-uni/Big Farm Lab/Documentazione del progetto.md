...
Dopo aver generato l'array con i parametri `params[]`, leggo i file da linea di comando e li copio su un array allocato (sfrutto il valore `optind`, rappresenta l’indice dell’elemento successivo in `argv`, in modo da sapere quanti file sono stati passati dalla linea di comando, e quindi quanto grande fare l'array).
## Segnali
...
- Altrimenti vuol dire che: 
	...
	- Inviare al client il `file_name:long` richiesto con l'invio del `long` dal client.