...
- Crea un proprio **connection socket** che rappresenta il punto terminale della sua connesione con il client. 
...
```
È un metodo della classe `ServerSocket`, mi viene restituita la socket particolare con cui comunicherò con il client che ha richiesto la connessione.
- Quando il processo server la invoca, pone il server **in attesa** di nuove connessioni.
- **Bloccante**: se non ci sono richieste, il server si blocca (possibile utilizzo di time-out).
...
public class DayTimeServer {  
	...
	try (ServerSocket server = new ServerSocket(PORT)) {
		while (true) {
			...
			// autoclose di conn perché stiamo usando try-with-resource
		}
	} catch (IOException ex) {
...
- Quando la variabile condivisa è di tipo semplice, altrimenti per acquisire la lock occorrerebbe fare il cast al corrispondente oggetto. 
