...
- Orari(CodAula, NomeAula, Piano, Posti, Materia, CDL, Docente, Giorno, OraInizio, OraFine)
	- In un dato momento, un docente si trova **al massimo** in un'aula $$\lnot((Giorno, DataInizio)_{=}\land Docente_{=}\implies Aula_\ne)\implies False$$ $$(Giorno, DataInizio)_{=}\land Docente_{=}\implies Aula_=$$
	- Non è possibile che **due docenti** condividano la **stessa aula** $$\lnot((Giorno, DataInizio)_{=}\land Docente_{\ne}\implies Aula_=)\implies False$$ $$(Giorno, DataInizio)_{=}\land Aula_=\implies Docente_=$$
	- Se **due lezioni** si svolgono su due piani diversi, allora appartengono a **due corsi diversi** $$Piano_{\ne}\implies CDL_{\ne} \iff CDL_{=} \implies Piano_{=}$$
	- Se **due lezioni diverse** si svolgono lo **stesso giorno** per la stessa materia, appartengono a **due CDL diversi** $$Lezioni_{\ne}\land Giorno_{=} \land Materia_{=}\implies CDL_\ne$$ $$Giorno_{=}\land Materia_{=}\land CDL_{=} \implies Lezioni_{=}$$ visto che $Lezioni$ non è un attributo, lo devo sostituire (assicurandomi che la dipendenza funzionale sia non banale) $$Giorno_{=}\land Materia_{=}\land CDL_{=} \implies X_{=}$$$$X\mid X = Orari \setminus \{Giorno,Materia,CDL\}$$
...
Per risolvere il problema dell'implicazione usiamo [[problema dell'implicazione#Algoritmo per calcolare la chiusura di $X$ rispetto a $F$|l'algoritmo per calcolare la chiusura]] di $X$ rispetto a $F$. 
