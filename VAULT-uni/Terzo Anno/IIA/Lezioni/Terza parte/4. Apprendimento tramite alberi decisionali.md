...
L'information gain favorisce gli attributi con molti valori possibili. Consideriamo l'attributo Data nell'esempio di PlayTennis. "Data" ha il massimo guadagno di informazioni: 

- Ogni giorno corrisponde a un sottoinsieme diverso che è puro: $[1+, 0-]$ o $[0+, 1-]$, cioè 0 entropia.
- Abbiamo perfetta separazione dei dati di training, ma pessima capacità di generalizzazione dato che "Data" parcellizza troppo (si fanno tanti sottoinsiemi piccolissimi 🥺).
```

### Gain Ratio
$$\text{GainRatio}(S,A) \equiv \frac{\text{Gain(S,A)}}{\text{SplitInformation}(S,A)} = \frac{\text{Entropy}(S) - \displaystyle\sum\limits_{v\in \text{Values}(A)} \dfrac{\lvert S_v \rvert}{\lvert S \rvert} \cdot \text{Entropy}(S_v)}{\text{SplitInformation}(S,A)}$$ con $$\text{SplitInformation}(S,A) = - \sum\limits_{i=1}^{c}\frac{\lvert S_{i}\rvert}{\lvert S\rvert} \lg \frac{\lvert S_{i}\rvert}{\lvert S\rvert}$$
...
La gain ratio penalizza gli attributi che dividono gli esempi in tante piccole classi, come Date che parcellizava troppo. 
- Sia $\lvert S\rvert = n$, Date divide gli esempi in $n$ sottoinsiemi. $$\begin{align*}
...
Il valore di $\text{SplitInformation}(S,A)$ può essere 0 o molto piccolo quando $\lvert S_{i}\rvert \approx \lvert S \rvert$ per qualche valore $i$. E così facendo il gain ratio esplode.
