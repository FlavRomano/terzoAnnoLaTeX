...
L'apprendimento consiste nella ricerca di una buona funzione in uno spazio di dati.
...
- Scarso fitting $\iff$ alto errore in training.
- Scarsa generalizzazione $\iff$ alto errore di test.
```
...
Stima delle performance (prestazioni) di modelli differenti di apprendimento per scegliere il migliore da generalizzare. Restituisce un modello.

n.b: include la ricerca dei parametri migliori per il modello.
```
...
```ad-def
title: Hold out
Hold-out è quando dividi il set di dati in un set di development (composto da due insiemi disgiunti, uno per il training e l'altro per la validation) e uno per il test. 
- Il set di development è ciò su cui viene calibrato il modello
- il set di test viene utilizzato per vedere come si comporta quel modello su dati non noti. 
```
...
### Separare TR, VL e TS
Dati 20 esempi e 1000 variabili di input random (con target binario), immaginiamo di avere un modello con una sola feature che indovina al 99\% sul dataset e poi su qualsiasi split successivo in TR, VL e TS.

- Va bene avere un modello con 99\% di accuratezza?
	- Assolutamente no, perché in primis non è un buon indice per la stima del rischio.
		-  Visto che la model assessment non c'entra nulla con il risk assessment.
...
In situazioni di questo tipo si è usato il test set all'inizio (spesso implicitamente).
...
- Addestriamo l'algoritmo di training su $D/D_{i}$ e testiamolo su $D_{i}$ (diagonale).
...
- **Model selection**. Usiamo K-fold CV sull'insieme TR, in questo modo otteniamo nuovi insiemi TR e VL per ogni split. Così dopo possiamo applicare una grid-search per cercare i migliori iperparametri possibili.
